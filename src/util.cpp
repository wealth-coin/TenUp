...
int nAnonymizeTenupAmount = 1000;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "tenup.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty tenup.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override tenup.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tenupd.pid"));
