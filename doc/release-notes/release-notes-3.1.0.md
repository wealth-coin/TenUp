WealthSilo Core version *3.1.0* is now available from:  <https://github.com/wealthsilo-project/wealthsilo/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/wealthsilo-project/wealthsilo/issues>
...
zWEALTH Staking is here! zWEALTH staking will be activated on the morning of the 8th of May 2018. With the release of zWEALTH staking, there are effectively 2 versions of zWEALTH, zWEALTH minted on the 3.0.6 WealthSilo wallet or lower, and zWEALTH minted on WealthSilo wallet version or higher. New features in this release will require the use of zWEALTH v2, zWEALTH minted on this wallet release 3.1.0 or later. If you currently hold zWEALTH v1 and wish to take advantage of zWEALTH staking and deterministic zWEALTH, you will need to spend the zWEALTH v1 to yourself and remint zWEALTH v2.
Note: To find your zWEALTH version, click the privacy tab, then the zWEALTH Control button then expand the arrows next to the desired denomination.


### Deterministic zWEALTH Seed Keys

zWEALTH is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zWEALTH outside of the wallet that the zWEALTH had been minted on. zWEALTH can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zWEALTH minting

zWEALTH minting now only requires 1 further mint (down from 2) to mature. zWEALTH mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zWEALTH Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zWEALTH denomination has been spent or not.



WEALTH/zWEALTH Staking and Masternode Rewards
--------------

### WEALTH, zWEALTH and Masternode Payment Schedule

To encourage the use of zWEALTH and increase the WealthSilo zerocoin anonymity set, the WealthSilo payment schedule has been changed to the following:

If a user staking zWEALTH wins the reward for their block, the following zWEALTH reward will be: 
- 3 zWEALTH (3 x 1 denominations) rewarded to the staker, 2 WEALTH rewarded to the masternode owner and 1 WEALTH available for the budget. This is a total block reward of 6 WEALTH, up from 5.

If a user staking WEALTH wins the reward, the following amounts will be rewarded: 
- 2 WEALTH to the WEALTH staker, 3 WEALTH to the Masternode owner and 1 WEALTH available for the budget. This is a total block reward of 6 WEALTH, up from 5.


### Return change to sender when minting zWEALTH

Previously, zWEALTH minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most WEALTH and return the change from a zWEALTH mint to that address.
...
-	Enable and disable the auto zWEALTH minting feature. This is enabled by default and the enablezeromint=0 setting in the wealthsilo.conf file will overwrite the GUI option.
-	The percentage of autominted zWEALTH can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular WEALTH.
...
### Summary
 ...
 The WealthSilo wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the wealthsilo.conf file, found in the WealthSilo data directory.
- backuppath = <directory / full path>
- zwealthbackuppath = <directory / full path>
...
* If zWEALTH backup, auto generated name is `wallet-autozwealthbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the WealthSilo support group, which often times sees users say "I didn't spend that zWEALTH". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the WealthSilo masternodes, the monthly budget available to be utilised has been increased to 42,000 WEALTH / month. This WEALTH only has the opportunity to be raised once per month (paid to winning proposals) with any unused WEALTH not created by the blockchain.

### Budget Finalization Fee

The WealthSilo finalization fee for successful proposals has now been reduced, this fee is now 5 WEALTH down from 50 WEALTH. The total fee outlay for a successful proposal is now a total of 55 WEALTH.
...
-	Enforced v1 zWEALTH spends to require a security level of 100
-	Updates to zWEALTH spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the Wealthsilo-Qt masternodes tab
-	Updated Wealthsilo-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and WealthSilo's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zWEALTH Control dialog in the QT wallet, a user is able to see zWEALTH marked as version 1 or 2.

zPoS (zWEALTH staking)
--------------

Once a zWEALTH has over 200 confirmations it becomes available to stake. Staking zWEALTH will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zWEALTH. So for example if a 1,000 zWEALTH denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zWEALTHs.
...
Zerocoins, or zWEALTH, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zWEALTH that can be recalculated at any time using the seed. Deterministic zWEALTH allows for users to backup all of their future zWEALTH by simply recording their seed and keeping it in a safe place (similar to backing up a private key for WEALTH). The zWEALTH seed needs to remain in the wallet in order to spend the zWEALTH after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zWEALTH data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.
...
### Core Features
 - #582 `cb1632520` [Core] zWEALTH v2: zPoS dzWEALTH ezWEALTH external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zWEALTH balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zWEALTH status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zWealth Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault wealthsilo-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zWEALTH spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zWEALTH. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zWEALTH. (presstab)
