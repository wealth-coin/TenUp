...
### Deterministic zTUP Seed Keys

zTUP is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zTUP outside of the wallet that the zTUP had been minted on. zTUP can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zTUP minting

zTUP minting now only requires 1 further mint (down from 2) to mature. zTUP mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zTUP Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zTUP denomination has been spent or not.



TUP/zTUP Staking and Masternode Rewards
--------------
...
If a user staking TUP wins the reward, the following amounts will be rewarded: 
- 2 TUP to the TUP staker, 3 TUP to the Masternode owner and 1 TUP available for the budget. This is a total block reward of 6 TUP, up from 5.


### Return change to sender when minting zTUP

Previously, zTUP minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most TUP and return the change from a zTUP mint to that address.
...
-	Enable and disable the auto zTUP minting feature. This is enabled by default and the enablezeromint=0 setting in the tenup.conf file will overwrite the GUI option.
-	The percentage of autominted zTUP can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular TUP.
...
* If zTUP backup, auto generated name is `wallet-autoztupbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
This allows running tenupd without having to do any manual configuration.
...
Once a zTUP has over 200 confirmations it becomes available to stake. Staking zTUP will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zTUP. So for example if a 1,000 zTUP denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zTUPs.
...
Zerocoins, or zTUP, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zTUP that can be recalculated at any time using the seed. Deterministic zTUP allows for users to backup all of their future zTUP by simply recording their seed and keeping it in a safe place (similar to backing up a private key for TUP). The zTUP seed needs to remain in the wallet in order to spend the zTUP after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zTUP data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.
...
### Core Features
 - #582 `cb1632520` [Core] zTUP v2: zPoS dzTUP ezTUP external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zTUP balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zTUP status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zTup Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault tenup-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zTUP spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zTUP. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zTUP. (presstab)
